...
function App() {
  ...
  const getJobs = useFetch({
    url: "https://mechio-api-test.onrender.composlovi",
    method: "get",
    onSuccess: (data) => {
      setJobs(data);
    },
  ...
  useEffect(() => {
    if (refetch) {
      getJobs.handleFetch("https://mechio-api-test.onrender.composlovi");
      getCompanies.handleFetch(
        "https://mechio-api-test.onrender.composlodavci"
      );
    }
    setRefetch(false);
  }, [refetch]);
